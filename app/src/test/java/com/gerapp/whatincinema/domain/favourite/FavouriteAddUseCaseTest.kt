...
internal class FavouriteAddUseCaseTest {
    ...
    @InjectMockKs
    private lateinit var favouriteAddUseCase: FavouriteAddUseCase

    @Test
    fun `Check that useCase trigger repository call`() = runTest {
        val testMovieId = 123
        favouriteAddUseCase(testMovieId).single()

        coVerifyOnce { repository.addMovieToFavourites(testMovieId) }
