...
internal class FetchFavouritesUseCaseTest {
    ...
    @InjectMockKs
    private lateinit var fetchFavouriteUseCase: FetchFavouritesUseCase

    @Test
    fun `Check that useCase trigger repository call`() = runTest {
        val testMovieId = 123
        fetchFavouriteUseCase(Unit).single()

        coVerifyOnce { repository.getFavouriteMoviesIds() }
