...
internal class FavouriteCheckUseCaseTest {
    ...
    @InjectMockKs
    private lateinit var favouriteCheckUseCase: FavouriteCheckUseCase

    @Test
    fun `Check that useCase trigger repository call`() = runTest {
        val testMovieId = 123
        favouriteCheckUseCase(testMovieId).single()

        coVerifyOnce { repository.checkIsMovieFavourite(testMovieId) }
